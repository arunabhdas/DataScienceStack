...
export default class Scientist extends mixins(JhiDataUtils) {
  ...
  public retrieveAllScientists(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.scientists.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IScientist): void {
  ...
  public removeScientist(): void {
    this.scientistService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A Scientist is deleted with identifier ' + this.removeId;
